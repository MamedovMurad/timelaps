...
type Props = {
    ...
    handleChangeServo: (param: string, callBack: (param: boolean) => void) => void
}

export default function CameraCard({ data, handleClickCamera, handleRemoveCamera, handleChangeServo }: Props) {
                    ...
                    : <div className="flex gap-2 justify-between">
                            ...
                            <div className="flex w-full mt-2 gap-x-2">
                                <div className=" bg-neytral-600 w-7/12 p-3 rounded-lg">
                                    ...
                                    <div className="flex  items-center mt-3 justify-between">
                                        ...
                                        {data?.type === "GOPRO" && <Switch className="custom-switch"
                                            defaultChecked={!!data.lastUsbCheckResult} size="small" onChange={() => handleChangeServo(data.id + "", setloading)} />}
                        ...
                        <div>
                            <ul className="flex flex-col gap-3 ">
                                ...
                                <li className=" bg-neytral-600 rounded-lg w-16 h-16 flex justify-center items-center flex-col">
                                    <CircleWarningIcon color={data?.lastPingValue && data.lastPingValue > 0 ? "#50C878" : undefined} />
                                    <span className=" text-white text-xs mt-1">{data?.lastPingValue && data.lastPingValue > 0 ? "Aktiv" : "Deaktiv"}  </span>
                                </li>
                                <li className=" bg-neytral-600 rounded-lg w-16 h-16 flex justify-center items-center flex-col">
                                    <CameraDisabledIocn />
